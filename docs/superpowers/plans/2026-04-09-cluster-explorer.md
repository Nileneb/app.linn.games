...
**Goal:** Integrate the Cluster Explorer 3D space game into the Laravel app at `/recherche/{projekt}/galaxy`, serving real pgvector-based galaxy data to the Three.js frontend.

**Architecture:** GalaxyController serves a fullscreen Blade view containing the complete Three.js game. A sibling web route `/recherche/{projekt}/galaxy-data` serves the galaxy JSON (auth-gated via ProjektPolicy). The Python generator (`scripts/cluster-explorer/generate_galaxy.py`) writes `public/galaxy-data/{projekt_id}.json`, triggered via Artisan command. The game falls back to hardcoded demo clusters when no JSON exists.

**Tech Stack:** Laravel 12 / PHP 8.4 / Pest / Three.js r128 (CDN) / Web Audio API / Python 3 + umap-learn (existing script)
...
| Status | File | Role |
|--------|------|------|
| Create | `app/Http/Controllers/GalaxyController.php` | Auth + policy check, passes `$projekt->id` to view |
| Create | `app/Http/Controllers/GalaxyDataController.php` | Returns `public/galaxy-data/{id}.json` or 404 JSON |
| Create | `app/Console/Commands/GenerateGalaxyData.php` | Artisan wrapper around Python script |
| Create | `resources/views/galaxy/show.blade.php` | Full game (adapted galaxy-v3.html) |
| Modify | `routes/web.php` | Add galaxy + galaxy-data routes inside auth group |
| Create | `tests/Feature/Galaxy/GalaxyControllerTest.php` | Feature tests for both HTTP endpoints |

---

## Task 1: GalaxyController + GalaxyDataController + Routes

**Files:**
- Create: `app/Http/Controllers/GalaxyController.php`
- Create: `app/Http/Controllers/GalaxyDataController.php`
- Modify: `routes/web.php`
- Create: `tests/Feature/Galaxy/GalaxyControllerTest.php`
...
beforeEach(function () {
    ...
    // Nutzer zum Workspace des Projekts hinzufügen
    $this->user->workspaces()->attach($this->projekt->workspace_id, ['role' => 'owner']);
});
...
test('galaxy data endpoint returns 404 json when no file exists', function () {
    // Sicherstellen, dass keine Datei existiert
...
```bash
docker compose run --rm php-test vendor/bin/pest tests/Feature/Galaxy/GalaxyControllerTest.php --no-coverage
```

Expected: all 6 tests FAIL (classes/routes don't exist yet).

- [ ] **Step 3: Create GalaxyController**
...
- [ ] **Step 4: Create GalaxyDataController**
...
In `routes/web.php`, inside the `Route::middleware(['auth', 'verified'])->group(function () {` block, after the export routes, add:
...
```bash
git add app/Http/Controllers/GalaxyController.php \
        app/Http/Controllers/GalaxyDataController.php \
        routes/web.php \
        tests/Feature/Galaxy/GalaxyControllerTest.php
git commit -m "feat: add GalaxyController, GalaxyDataController and routes"
```

---

## Task 2: Artisan Command galaxy:generate

**Files:**
- Create: `app/Console/Commands/GenerateGalaxyData.php`
- Create: `tests/Feature/Galaxy/GenerateGalaxyDataCommandTest.php`
...
test('galaxy:generate outputs error when python script not found', function () {
    ...
    // Script-Pfad überschreiben auf nicht existierenden Pfad
...
```bash
docker compose run --rm php-test vendor/bin/pest tests/Feature/Galaxy/GenerateGalaxyDataCommandTest.php --no-coverage
```

Expected: FAIL (command doesn't exist).

- [ ] **Step 3: Add galaxy config key**

In `config/services.php`, add at the end of the file (before the final `]`):
...
class GenerateGalaxyData extends Command
{
    ...
    public function handle(): int
    {
        ...
        // Shell-Escaping für alle Argumente
        $cmd = sprintf(
...
```bash
git add app/Console/Commands/GenerateGalaxyData.php \
        config/galaxy.php \
        tests/Feature/Galaxy/GenerateGalaxyDataCommandTest.php
git commit -m "feat: add galaxy:generate artisan command"
```

---

## Task 3: Galaxy Blade View (Full Game)

**Files:**
- Create: `resources/views/galaxy/show.blade.php`
- Modify: `resources/views/recherche/show.blade.php` (link hinzufügen)

No separate test needed — the view is covered by the controller tests in Task 1. Smoke-test via browser.

- [ ] **Step 1: Create the Blade view**

Create `resources/views/galaxy/show.blade.php`.

This is a standalone fullscreen HTML document (no `@extends`). It adapts galaxy-v3.html: the hardcoded `clusters` array is replaced by async fetch from the galaxy-data endpoint, with fallback to demo clusters.
...
// Normalisiert galaxy-data.json cluster → game cluster format
...
async function loadClusters() {
  try {
    ...
    if (!data.clusters || data.clusters.length === 0) throw new Error('empty');
...
// Stars (static, built immediately)
...
// Planets built after data loads
  ...
  if(dist<60){
    ...
    if(!e.userData.wanderTarget||e.userData.wanderTimer<=0){
      e.userData.wanderTarget=new THREE.Vector3(
        e.position.x+(Math.random()-.5)*60,
        e.position.y+(Math.random()-.5)*20,
        e.position.z+(Math.random()-.5)*60);
...
function spawnEnemy(type='anomaly',pos=null){
  ...
  if(type==='anomaly'){
    ...
    hp=1;pts=100;size=1.4;speed=.05+wave*.012;
  }else if(type==='collision'){
    geo=new THREE.TetrahedronGeometry(2.2,0);
    mat=new THREE.MeshPhongMaterial({color:0xf97316,emissive:0xf97316,emissiveIntensity:.6});
    hp=2;pts=250;size=2.2;speed=.06+wave*.015;
  }else if(type==='dark'){
    geo=new THREE.BoxGeometry(1.8,1.8,1.8);
    mat=new THREE.MeshPhongMaterial({color:0x8b5cf6,emissive:0x8b5cf6,emissiveIntensity:.5,transparent:true,opacity:.85});
    hp=3;pts=400;size=1.8;speed=.09+wave*.018;
  }else if(type==='boss'){
    geo=new THREE.IcosahedronGeometry(6,1);
    mat=new THREE.MeshPhongMaterial({color:0xff2200,emissive:0xff2200,emissiveIntensity:.35,shininess:120});
    hp=15+wave*5;pts=3000;size=6;speed=.04+wave*.008;
  }
  const mesh=new THREE.Mesh(geo,mat);
  if(pos)mesh.position.copy(pos);
  else{
    const r=80+Math.random()*50,a=Math.random()*Math.PI*2,b=(Math.random()-.5)*.8;
    mesh.position.set(camera.position.x+r*Math.cos(a)*Math.cos(b),camera.position.y+r*Math.sin(b)*20,camera.position.z+r*Math.sin(a)*Math.cos(b));
  }
  mesh.userData={type,hp,maxHp:hp,pts,size,speed,
    vel:new THREE.Vector3((Math.random()-.5)*.02,(Math.random()-.5)*.02,(Math.random()-.5)*.02),
    spinX:(Math.random()-.5)*.07,spinY:(Math.random()-.5)*.08,
    teleportTimer:type==='dark'?80+Math.random()*40:999999,
...
function fireLaser(){
  ...
  l.userData={dir,speed:5,life:80};
...
function startWave(n){
  ...
  for(let i=0;i<total;i++){
    setTimeout(()=>{
      const r=Math.random();
      spawnEnemy(n>=4&&r<.08?'boss':r<.2?'dark':r<.45?'collision':'anomaly');
    },i*350);
  }
}
...
function animate(){
  ...
  if(gameStarted){
    ...
    if(spawnTimer>interval){
      spawnTimer=0;
      const r=Math.random();
      spawnEnemy(r<.15?'dark':r<.4?'collision':'anomaly');
      if(enemies.length<4)spawnEnemy('anomaly');
    }
    ...
    for(let i=lasers.length-1;i>=0;i--){
      ...
      for(let j=enemies.length-1;j>=0;j--){
        ...
        if(l.position.distanceTo(enemies[j].position)<(enemies[j].userData.size+1.5)){
          hits++;
          const e=enemies[j];e.userData.hp--;
          ...
          if(e.userData.hp<=0){
            ...
            soundHit();addScore(e.userData.pts);
            kills++;waveKills++;killsEl.textContent=kills;
...
- [ ] **Step 2: Add Galaxy link to project show page**

In `resources/views/recherche/show.blade.php`, find the section with export/action buttons (search for `export` or the button group). Add a Galaxy link alongside existing buttons:
...
<a href="{{ route('recherche.galaxy', $projekt) }}"
   ...
   title="Cluster Explorer — 3D Galaxy">
    ✦ Galaxy
</a>
```

The exact insertion point depends on the current layout. Find a logical button group in `show.blade.php` and add the link there. If there's an `<x-export-button-group>` or similar component, add it after it.
...
Navigate to any project page → click "Galaxy" link → confirm:
- Start overlay appears with "Lade Galaxie-Daten..." → changes to "Demo-Modus" or cluster count
- Start button becomes enabled
- Clicking Start launches the game
- WASD/Mouse controls work
- Enemies spawn and chase
- Laser fires on click/space
- Explosions + sound work

- [ ] **Step 4: Commit**

```bash
git add resources/views/galaxy/show.blade.php \
        resources/views/recherche/show.blade.php
git commit -m "feat: add Cluster Explorer galaxy view with Three.js game"
```
...
Expected: all existing tests pass + new 9 galaxy tests pass.
...
If linting fails, fix with:

```bash
vendor/bin/pint
git add -A
git commit -m "style: pint autofix"
```
...
**Generate galaxy data for a project:**

```bash
php artisan galaxy:generate <projekt-uuid>
# e.g.: php artisan galaxy:generate 018e4b2c-1234-7000-abcd-ef0123456789
```
...
The script reads the project's `.env` for DB credentials automatically.
