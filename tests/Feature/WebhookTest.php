...
test('webhook rejects request with missing eingabe field', function () {
    Queue::fake();
    Config::set('services.langdock.secret', 'test-secret');
...
test('webhook rejects request with non-existent projekt_id', function () {
    Queue::fake();
    Config::set('services.langdock.secret', 'test-secret');
    ...
    $payload = [
        'user_id' => $user->id,
        'projekt_id' => fake()->uuid(), // does not exist in DB
        'eingabe' => 'Testfrage',
    ];
    $body = json_encode($payload);
    $signature = hash_hmac('sha256', $timestamp . '.' . $body, 'test-secret');
...
test('webhook rejects request with non-existent user_id', function () {
    Queue::fake();
    Config::set('services.langdock.secret', 'test-secret');

    $projekt = \App\Models\Recherche\Projekt::factory()->create();
...
test('webhook rejects request with eingabe exceeding max length', function () {
    Queue::fake();
    Config::set('services.langdock.secret', 'test-secret');
    ...
    $payload = [
        'user_id' => $user->id,
        'projekt_id' => $projekt->id,
        'eingabe' => str_repeat('x', 10001), // exceeds max:10000
    ];
    $body = json_encode($payload);
    $signature = hash_hmac('sha256', $timestamp . '.' . $body, 'test-secret');
