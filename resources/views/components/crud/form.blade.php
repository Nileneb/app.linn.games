...
@if ($visible)
    <div class="fixed inset-0 z-30 bg-black/50 backdrop-blur-sm" wire:click="{{ $cancelAction }}"></div>
    <div class="fixed inset-y-0 right-0 z-40 flex w-full flex-col overflow-hidden bg-white shadow-2xl dark:bg-neutral-900 sm:max-w-md">
        <div class="flex items-center justify-between border-b border-neutral-200 px-4 py-3 dark:border-neutral-700">
            ...
            <button wire:click="{{ $cancelAction }}" class="rounded p-1 text-neutral-400 hover:bg-neutral-100 hover:text-neutral-700 dark:hover:bg-neutral-800 dark:hover:text-neutral-200">
        ...
        <div class="flex-1 overflow-y-auto px-4 py-4 space-y-4 dark:text-neutral-100">
            {{-- Form Fields Slot --}}
            {{ $slot }}
        </div>
        <div class="border-t border-neutral-200 px-4 py-3 dark:border-neutral-700">
            <div class="flex justify-end gap-2">
                <button wire:click="{{ $cancelAction }}" class="rounded px-4 py-2 text-sm text-neutral-600 hover:bg-neutral-100 dark:text-neutral-300 dark:hover:bg-neutral-800">{{ $cancelLabel }}</button>
